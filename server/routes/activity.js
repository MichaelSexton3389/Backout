...
activityRouter.post("/api/activities", authMiddleware, async (req, res) => {
    try {
        const { title, description,bg_img, location, date } = req.body;
        ...
        // Append the created activity to the user's activities array
        await User.findByIdAndUpdate(userId, { $push: { activities: savedActivity._id } });
